...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < 16; i++) 
    {
        ...
        {       
            ...
            printf("count:        %lums A: %ld, C: %ld, G: %ld, T: %ld\n", 
                timeElapsed / 1000000,
                values['A'],
                values['C'],
                values['G'],
        ...
        {       
            ...
            uint64_t timeElapsed = timespecDiff(&end, &start);
            printf("countAVX2:    %lums A: %ld, C: %ld, G: %ld, T: %ld\n", 
                timeElapsed / 1000000,
                values['A'],
                values['C'],
                values['G'],
        ...
        {       
            ...
            uint64_t timeElapsed = timespecDiff(&end, &start);
            printf("countAVX2_1:  %lums A: %ld, C: %ld, G: %ld, T: %ld\n", 
                timeElapsed / 1000000,
                values['A'],
                values['C'],
                values['G'],
...
void fill(char* data,const size_t size) 
{
    uint64_t* dptr = (uint64_t)data;

    static runid = 0;
    runid++;

    srand(0x13377331 + runid);

    for(size_t i = 0; i < size / sizeof(uint64_t); i++)
    {
        dptr[i] = qwords[rand() % (sizeof(qwords) / sizeof(qwords[0]))];
